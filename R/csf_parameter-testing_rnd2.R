...
#
# Round two of testing: Iterates over parameter range that produced low RMSE from
# round one. User should conduct testing one parameter at a time, then update that
# parameter's value to value corresponding to new min RMSE before proceeding. Otherwise,
...
cr_testrange <- seq(0.3, 0.6, 0.01)
ct_testrange <- seq(0, 0.09, 0.01)
ts_testrange <- seq(0.4, 1, 0.01)
n_testrange <- seq(0, 0.7, 0.01)
...
output <- glue('data/dtm/csf_parameter-testing/ppwd_csf-parameter-testing_rnd{round}_{test_param}_z{zone}.csv')
...
if (test_param == 'cloth-resolution') {
